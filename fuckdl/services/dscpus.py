...
class DSCPUS(BaseService):
    """
    Service code for Discovery+ US (https://www.discoveryplus.com/).
    ...
    ALIASES = ["dscpus"]
    TITLE_RE = r"^(?:https?://(?:www\.|play\.)?discoveryplus\.com/)?(?P<type>[^/]+)/(?P<id>[^/]+)"
    ...
    @staticmethod
    @click.command(name="DSCPUS", short_help="discoveryplus.com")
    @click.argument("title", type=str, required=False)

    @click.pass_context
    def cli(ctx, **kwargs):
        return DSCPUS(ctx, **kwargs)
    ...
    def get_titles(self):
        ...
        response = self.session.get(
            f"https://default.any-any.prd.api.discoveryplus.com/cms/routes/{content_type}/{external_id}?include=default",
        )
        ...
        if content_type == "movie" or content_type == "standalone":
            metadata = self.session.get(
                url=f"https://default.any-any.prd.api.discoveryplus.com/content/videos/{external_id}/activeVideoForShow?&include=edit"
            ).json()['data']
        ...
        if content_type in ["show", "mini-series", "topical"]:
            ...
            for (value, parameter) in season_parameters:
                data = self.session.get(url="https://default.any-any.prd.api.discoveryplus.com/cms/collections/generic-show-page-rail-episodes-tabbed-content?include=default&pf[show.id]=%s&%s" % (external_id, parameter)).json()
                try:
    ...
    def get_tracks(self, title):
        ...
        try:
            self.license_url = playback_data["drm"]["schemes"]["playready"]["licenseUrl"] if self.playready else playback_data["drm"]["schemes"]["widevine"]["licenseUrl"]
    ...
    def get_device_token(self):
        response = self.session.post(
            'https://default.any-any.prd.api.discoveryplus.com/session-context/headwaiter/v1/bootstrap',
        )
